  ...
  async uploadFile(bodycontent) {
    ...
    s3.putObject(params, function(err, data) {
      if (err)
        ...
        console.log("Successfully uploaded fixity output file to " + outputBucket + "/" + outputFilename);
    });

  }

  async storeFixityOutput() {
    const {
      base,
    } = PATH.parse(this.key);

    const statusResult = `${(this.algorithm || 'Checksum').toUpperCase()} status on ${base}`;
